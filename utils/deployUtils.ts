...
const deployVaultAndStrategy = async (
  ...
  const VaultFactory = await ethers.getContractFactory(contractConfig.vault);
...
const updateFenderVaults = (minimumVault) => {
  ...
  const spaPool = fantomPools.filter((pool) => pool.id == "spartacus-spa")[0];
