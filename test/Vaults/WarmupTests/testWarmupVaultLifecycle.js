...
describe(WARMUP_TEST_FLAG + VAULT_TEST_FLAG + " Lifecycle", function () {
  let vault,
    strategy,
    unirouter,
    unirouterData,
  ...
  it("Warmup is properly configured", async function () {
    expect(await stakeManager.warmupPeriod()).to.eq(1);
  ...
  it(
    ...
    async function () {
      ...
      let warmupBalance = (await stakeManager.warmupInfo(strategy.address))
        .deposit;

      expect(warmupBalance)
      ...
      expect(await strategy.rebaseBonded()).to.eq(
        vaultBal.sub((await stakeManager.warmupInfo(strategy.address)).deposit)
      );
      ...
      // travel to the end of warmup
      await timeTravelBlocks(ethers.provider, REBASE_PERIOD_BLOCKS);

      let currentReservePeriod = await strategy.currentReservePeriod();
  ...
  it(
    ...
    async function () {
      ...
      let warmupBal = (await stakeManager.warmupInfo(strategy.address)).deposit;

      expect(warmupBal).to.gte(pendingPayout.mul(999).div(1000));
      ...
      warmupBal = (await stakeManager.warmupInfo(strategy.address)).deposit;

      expect(await strategy.rebaseBonded()).to.gt(wantBalStart.sub(warmupBal));
  ...
  it(
    ...
    async function () {
      ...
      const warmupBal = (await stakeManager.warmupInfo(strategy.address))
        .deposit;
  ...
  it(
    ...
    async function () {
      ...
      expect(await strategy.unstakedRebasing()).to.eq(0);
      expect(warmupBal)
        .to.lte((await strategy.rebaseBonded()).add(1))
        .to.gte((await strategy.rebaseBonded()).sub(1));
      ...
      expect(vaultBalAfterRebase)
        .to.eq(warmupBal.add(await strategy.rebaseBonded()))
        .to.eq(vaultBalAfterFirstRedeem); // notice that we only realize rebase gains after claim
      ...
      expect(vaultBalFinal).to.eq(warmupBal);
      expect(await strategy.rebaseBonded()).to.eq(0);

      // travel to the end of warmup
      await timeTravelBlocks(ethers.provider, REBASE_PERIOD_BLOCKS * 2);
  ...
  it(
    ...
    async function () {
      ...
      const warmupBal = await strategy.warmupBalance();

      expect(warmupBal)
        .to.gt(0)
        .to.eq(vaultBalAfterFirstRedeem.sub(await strategy.rebaseBonded()));
      expect(await strategy.totalRebasing()).to.eq(0);
      expect(warmupBal).to.lte((await strategy.rebaseBonded()).add(1));
      expect(warmupBal).to.gte((await strategy.rebaseBonded()).sub(1));
      ...
      // Expect vault balance to not increase after rebase, realized after claim
      expect(vaultBalAfterRebase)
        .to.eq(
          (await strategy.totalRebasing())
            .add(await strategy.rebaseBonded())
            .add(await strategy.warmupBalance())
        )
        .to.eq(vaultBalAfterFirstRedeem);

      const before = await vault.balance();
      await strategy.redeemAndStake(); // final redemption
      const rebaseDiff = (await vault.balance()).sub(before);

      expect(await vault.balance()).to.eq(
        (await strategy.warmupBalance()).add(await strategy.totalRebasing())
      );

      await strategy.unstake(wantBalStart);

      let vaultBalFinal = await vault.balance();

      expect(vaultBalAfterRebase.add(rebaseDiff)).to.eq(vaultBalFinal);
      expect(await strategy.rebaseBonded()).to.eq(0);
      ...
      vaultBalFinal = await vault.balance();
