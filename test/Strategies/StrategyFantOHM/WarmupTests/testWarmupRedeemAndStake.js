...
describe(
  FANTOHM_TEST_FLAG + WARMUP_TEST_FLAG + " Strategy redeemAndStake",
  function () {
    let vault,
      strategy,
      unirouter,
    ...
    it(
      ...
      async function () {
        ...
        const staked = await strategy.stakedRebasing();
        const warmup = pendingPayout;
        ...
        expect(await strategy.warmupBalance()).to.equal(pendingPayout);
        expect(await strategy.totalRebasing()).to.equal(0);
        expect(stakedFhmBal).to.eq(0);
        ...
        await timeTravelBlocks(
          ...
          parseInt(
            ethers.utils.formatUnits(
              bondDetailsInitial.vesting.mul(2).div(15),
              0
            )
          )
        );
        ...
        let stratBalBeforeRedeem = (
          await stakeManager.warmupInfo(strategy.address)
        ).deposit;
    ...
    it(SLOW_TEST_FLAG + "Two bonds back to back test", async function () {
      ...
      // Bonding again fails if we haven't warmed up
      await expect(
      ...
      let currentEpochNumber = (await stakeManager.epoch()).number;

      expect(whaleReservePeriodNumber).to.eq(1);
      expect(whaleReservePeriod.fullyVested).to.be.true;
      expect(whaleReservePeriod.warmupExpiry).to.eq(currentEpochNumber.add(1));
      ...
      expect(whaleReservePeriod.warmupExpiry).to.eq(
      ...
      currentEpochNumber = (await stakeManager.epoch()).number;

      expect(whaleReservePeriodNumber).to.eq(1);
      expect(whaleReservePeriod.fullyVested).to.be.true;
      expect(whaleReservePeriod.warmupExpiry).to.eq(currentEpochNumber);

      expect(whaleReservePeriod.warmupExpiry).to.eq(
        (await stakeManager.epoch()).number
      );

      expect(await strategy.claimerVested(whale._address)).to.be.true;
    ...
    it(
      ...
      async function () {
        ...
        let deployerWithdrawalFeeAmount = deployerWithdrawAmount
          .mul(withdrawalFee)
          .div(withdrawalFeeDenom);
        const deployerFirstWithdrawAmount = deployerWithdrawAmount.sub(
          deployerWithdrawalFeeAmount
        );

        expect(deployerFirstClaim.amount.add(whaleFirstClaim.amount)).to.eq(
          await strategy.reserves()
        );
        expect(deployerFirstClaim.amount).to.eq(deployerFirstWithdrawAmount);
        ...
        expect(deployerReservePeriod.warmupExpiry).to.eq(
          currentEpochNumber.add(1)
        );
        ...
        expect((await strategy.claimOfReserves(whale._address)).index).to.eq(1);
        expect((await strategy.claimOfReserves(deployer.address)).index).to.eq(
          2
        );

        const vaultBal2 = await vault.balance();
        expect(vaultBal2).to.eq(
          vaultBalStart
            .sub(withdrawAmount)
            .sub(deployerWithdrawAmount.sub(deployerWithdrawalFeeAmount))
        );

        // Deployer reserves instead of claiming, now full position will be vested out when warmup ends
        ...
        const deployerReserveBal = deployerClaim.amount;
        expect(deployerReservePeriod.warmupExpiry)
          .to.eq(
            (await stakeManager.epoch()).number.add(
              await stakeManager.warmupPeriod()
            )
          )
          .to.eq(await strategy.currentWarmupExpiry());

        expect(deployerReserveBal)
          .to.eq(
            deployerFirstWithdrawAmount.add(
              vaultBal2
                .div(2)
                .sub(
                  vaultBal2.mul(withdrawalFee).div(2).div(withdrawalFeeDenom)
                )
            )
          )
        ...
        // Must finish warming before we stakeToBond again
        await timeTravelBlocks(ethers.provider, REBASE_PERIOD_BLOCKS);
        await stakeManager.rebase();
        const currentEpoch = (await stakeManager.epoch()).number;
        const warmupExpiry = await strategy.currentWarmupExpiry();

        expect(currentEpoch).to.eq(warmupExpiry);
        ...
        expect(deployerReservePeriodNumber).to.eq(1);
        expect(deployerReservePeriod.warmupExpiry).to.eq(currentEpochNumber);
        expect(deployerReservePeriod.fullyVested).to.be.true;
        ...
        expect(await strategy.reserves()).to.eq(
          whaleSecondClaim.amount.add(deployerReserveBal)
        );
        ...
        expect(deployerClaimAfter.amount).to.eq(deployerReserveBal);
        ...
        expect(await strategy.reserves()).to.eq(deployerReserveBal);
        ...
        // deployer gets paid
        expect(deployerBal).to.eq(deployerReserveBal);
        expect(deployerBal).to.gt(rebaseTokenBalStart);
    ...
    it.skip(
      ...
      async function () {
        ...
        await strategy.redeemAndStake();
        let stakedFhmBal = await strategy.warmupBalance();
        ...
        let stratBalBeforeRedeem = await strategy.warmupBalance();
