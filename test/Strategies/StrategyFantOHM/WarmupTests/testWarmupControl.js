...
describe(
  FANTOHM_TEST_FLAG + WARMUP_TEST_FLAG + " Strategy Control Functions",
  function () {
    let vault,
      strategy,
      unirouter,
    ...
    it("Can pause so that users can only withdraw and then unpause", async function () {
      ...
      await stakedFhm.connect(whale).transfer(deployer.address, rebaseBal);
      ...
      expect(await fhm.balanceOf(deployer.address)).to.equal(
        rebaseBal
          .add(rebaseTokenBalStart)
          .sub(rebaseTokenBalStart.mul(withdrawalFee).div(withdrawalFeeDenom))
      );
    }).timeout(TEST_TIMEOUT);
    ...
    it("Can't pause when paused and unpause when unpaused", async function () {
      expect(await strategy.paused()).to.be.false;
      await expect(strategy.unpause()).to.be.revertedWith(
        "Pausable: not paused"
      );
      expect(await strategy.paused()).to.be.false;
      await strategy.pause();
    ...
    it(
      ...
      async function () {
        ...
        const warmingFunds = await strategy.warmupBalance();

        await strategy.panic();

        expect(stratfhmRedeemed).to.eq(0);

        expect(await strategy.stakedRebasing())
          .to.equal(0)
          .to.equal(await stakedFhm.balanceOf(strategy.address));
        expect(await strategy.unstakedRebasing())
          .to.equal(warmingFunds.add(availablePayout))
          .to.equal(await fhm.balanceOf(strategy.address));

        const stratfhm = await strategy.unstakedRebasing();

        // totalBalance should be either stuck bonding or unstaked
        expect(await strategy.totalBalance()).to.equal(
          stratfhm.add(await strategy.rebaseBonded())
        );
      }
    ...
    it("Manager can force unstake and stake", async function () {
      ...
      expect(await stakedFhm.balanceOf(strategy.address)).to.eq(0);

      expect(await strategy.warmupBalance())
        .to.equal(await vault.balance())
        .to.equal(rebaseTokenBalStart);

      expect(await strategy.currentWarmupExpiry()).to.eq(
        (await stakeManager.epoch()).number.add(3)
      );
    ...
    it("Can retrieve bespoke tokens, but not want or stakedWant", async function () {
      ...
      await expect(
        strategy.inCaseTokensGetStuck(STAKED_FHM)
    ...
    it("Staking when a reserve period is finished, begins a new reserve period with just warmup", async function () {
      ...
      expect(await strategy.reservePeriodFinished(newReservePeriod)).to.be
        .false;
    }).timeout(TEST_TIMEOUT);
    ...
    it(
      ...
      async function () {
        ...
        expect(reservePeriod.warmupExpiry)
          .to.eq(currentEpoch.add(warmupPeriod.sub(1)))
          .to.eq(predictedWarmupExpiry);
        expect(reservePeriod.fullyVested).to.be.true;
        expect(await strategy.reservePeriodFinished(currentReservePeriod)).to.be
          .false;
