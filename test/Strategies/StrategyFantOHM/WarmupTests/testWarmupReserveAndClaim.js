...
describe(
  FANTOHM_TEST_FLAG + WARMUP_TEST_FLAG + " Strategy Reserve/Claim",
  function () {
    let vault,
      strategy,
      unirouter,
    ...
    it("Cannot reserve immediately while bonding", async function () {
      ...
      expect((await strategy.claimOfReserves(deployer.address)).amount)
        ...
        .to.gt(rebaseTokenBalStart.sub(withdrawalFeeAmount));
    ...
    it("Over one bond withdraw 1/3, 1/3, then receive ~2/3 on final redemption", async function () {
      ...
      const withdrawalFee = await strategy.withdrawalFee();
      const withdrawalFeeDenom = await strategy.WITHDRAWAL_FEE_DIVISOR();
      let vaultBal = await strategy.rebaseBonded();
      ...
      let withdrawalAmount = vaultBal.div(3);
      ...
      const currentStratWarmup = await strategy.currentWarmupExpiry();

      expect(currentEpoch).to.eq(currentStratWarmup);
