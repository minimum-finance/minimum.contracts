...
describe("Spartacus Strategy redeemAndStake", function () {
  let vault,
    strategy,
    unirouter,
    spa,
  ...
  it(SLOW_TEST_FLAG + "Two bonds back to back test", async function () {
    ...
    // whale gets paid
    expect(whaleBal.sub(whaleBalInitial.sub(rebaseTokenBalStart))).to.eq(
      withdrawalAmountWhale.sub(withdrawalFeeAmount)
    );
